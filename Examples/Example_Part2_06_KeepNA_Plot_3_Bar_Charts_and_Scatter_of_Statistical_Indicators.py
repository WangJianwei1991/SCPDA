...
def KeepNA_Plot_3_Bar_Charts_and_Scatter_of_Statistical_Indicators(
    ...
    BatchCorrection = ['NoBC', 'limma']   # ['NoBC', 'limma', 'Combat-P', 'Combat-NP', 'Scanorama']
    StatisticalTest = ['t-test', 'Wilcox', 'limma-trend']  # ['t-test', 'Wilcox', 'limma-trend', 'limma-voom', 'edgeR-QLF', 'edgeR-LRT', 'DESeq2']
    ...
    for pValue in pValue_List:
        for SR in SR_methods:
            for Compared_groups in Compared_groups_label:
                ...
                fig, ax = plt.subplots(len(Normalization), len(BatchCorrection), figsize=(5,2.5))
                ...
                plt.subplots_adjust(left=0.08, right=0.90, bottom=0.01, top=0.80, wspace=0.05)
    ...
    # Draw bar chart of UseCov, corresponding to KeepNA (Two batch corrections, three statistical tests)
    ...
    for pValue in pValue_List:
        for SR in SR_methods:
            for Compared_groups in Compared_groups_label:
                ...
                # The top ranked method after removing KeepNA
                df_p_SR_NoKeepNA = df_p_SR[df_p_SR['Missing Value Imputation'] != 'KeepNA']

                top1_Imputation_method = df_p_SR_NoKeepNA['Missing Value Imputation'].values.tolist()[0] # Best Missing Value Imputation Method
...
if __name__ == '__main__': 
    KeepNA_Plot_3_Bar_Charts_and_Scatter_of_Statistical_Indicators(
