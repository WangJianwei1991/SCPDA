...
def NoCov_Plot_1_Parallel_Coordinates_Plots_and_Frequency_Bar_Charts(
    # The NoCov table selects the data of 3 batch effect correction methods that do not use covariates, and the result file after merging with the UseCov table
    result_csv_path = "E:\WJW_Code_Hub\SCPDA\Report_Results_Part2\DIANN_QC_3Mix_UsePCA_NoCovariates\Results\MethodSelection_DifferentialExpressionAnalysis-AddUseCov.csv",
    ...
    BatchCorrection = ['NoBC', 'limma', 'Combat-P', 'Combat-NP', 'Scanorama', 'limma-NC', 'Combat-P-NC', 'Combat-NP-NC']
    ...
    for P_value in pValue_List:
        for SR_method in SR_methods:
            ...
            for Compared_groups in Compared_groups_label:
                ...
                # 'limma-NC', 'Combat-P-NC', 'Combat-NP-NC'
                filtered_df = df_screened[df_screened['Batch Correction'].isin(['limma-NC', 'Combat-P-NC', 'Combat-NP-NC'])]
                filtered_df = filtered_df.sort_values(Compared_groups + ' Rank', ascending=True)
                NoCovariate_No_list = filtered_df['No'].values.tolist()

                # 'limma', 'Combat-P', 'Combat-NP'
                filtered_df = df_screened[df_screened['Batch Correction'].isin(['limma', 'Combat-P', 'Combat-NP'])]
                filtered_df = filtered_df.sort_values(Compared_groups + ' Rank', ascending=True)
                UseCovariate_No_list = filtered_df['No'].values.tolist()

                # 'NoBC', 'Scanorama'
                filtered_df = df_screened[df_screened['Batch Correction'].isin(['NoBC', 'Scanorama'])]
                filtered_df = filtered_df.sort_values(Compared_groups + ' Rank', ascending=True)
                BC_and_Scan_No_list = filtered_df['No'].values.tolist()
                ...
                for rank in current_rank_list:
                    ...
                    if rank_ratio == 0:
                        ...
                        new_rank_list.append(int((7840/6)*rank_ratio/0.01))
                    elif rank_ratio <= 0.02:
                        new_rank_list.append(int((7840/6)*1+(7840/6)*(rank_ratio-0.01)/0.01))
                    elif rank_ratio <= 0.03:
                        new_rank_list.append(int((7840/6)*2+(7840/6)*(rank_ratio-0.02)/0.01))
                    elif rank_ratio <= 0.04:
                        new_rank_list.append(int((7840/6)*3+(7840/6)*(rank_ratio-0.03)/0.01))
                    elif rank_ratio <= 0.05:
                        new_rank_list.append(int((7840/6)*4+(7840/6)*(rank_ratio-0.04)/0.01))
                    else:
                        new_rank_list.append(int((7840/6)*5+(7840/6)*(rank_ratio-0.05)/0.95))
                ...
                if Use_Given_PValue_and_FC:
                    ys = np.append(ys, [[0, 0, 0, 0, ys.max(axis=0)[4], ys.max(axis=0)[5], ys.max(axis=0)[6], ys.max(axis=0)[7], ys.max(axis=0)[8], ys.max(axis=0)[9], 7840]], axis=0)
                    ...
                    ys = np.append(ys, [[0, 0, 0, 0, ys.max(axis=0)[4], ys.max(axis=0)[5], ys.max(axis=0)[6], ys.max(axis=0)[7], ys.max(axis=0)[8], 7840]], axis=0)
                ...
                for plot_index in range(2):
                    ...
                    for i, ax in enumerate(axes):
                        ...
                        if ax != host:
                            ...
                            if i == 1:
                                ...
                                ax.set_yticks([0, (7840/6)*1, (7840/6)*2, (7840/6)*3, (7840/6)*4, (7840/6)*5, 7840])
                    ...
                    for tuceng in range(4):
                        for j in range(ys.shape[0] - 2):
                            ...
                            if (current_No in NoCovariate_No_list):

                                current_No_rank = NoCovariate_No_list.index(current_No)/(len(NoCovariate_No_list))

                                # Top3 Red Bold
                                if (NoCovariate_No_list.index(current_No) <= 2):
                                    if (tuceng != 3):
                                        continue
                                    edgecolor = '#b0203f'
                                    linewidth = 2.0
            ...
            # 'limma-NC', 'Combat-P-NC', 'Combat-NP-NC'
            filtered_df = df_screened[df_screened['Batch Correction'].isin(['limma-NC', 'Combat-P-NC', 'Combat-NP-NC'])]
            filtered_df = filtered_df.sort_values('Average Rank', ascending=True)
            NoCovariate_No_list = filtered_df['No'].values.tolist()
...
if __name__ == '__main__': 
    NoCov_Plot_1_Parallel_Coordinates_Plots_and_Frequency_Bar_Charts(
