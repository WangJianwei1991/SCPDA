...
def venn2(labels, names=['A', 'B'], **options):
    ...
    colors_used = []
    for name in names:
        if name in ['LibraryFree', 'directDIA', 'Discovery']:
            colors_used.append(default_colors[0])

        elif name == 'DDALib':
            colors_used.append(default_colors[1])

        elif name == 'PublicLib':
            colors_used.append(default_colors[2])

        elif name == 'AlphaPeptDeep':
            colors_used.append(default_colors[3])

        else:
            colors_used.append(default_colors[names.index(name)])

    if names == ['DIA-NN', 'PEAKS']:
        colors_used = [default_colors[0], default_colors[2]]


    colors = options.get('colors', colors_used)
    figsize = options.get('figsize', (4, 4))
    dpi = options.get('dpi', 600)
    fontsize = options.get('fontsize', 16)
    legend_fontsize = options.get('legend_fontsize', 18)
    ...
    # legend
    if len(names[0]) > 10:
        draw_text(fig, ax, 0.25, 0.7, names[0], colors[0], fontsize=legend_fontsize, ha="center")
    else:
        draw_text(fig, ax, 0.2, 0.7, names[0], colors[0], fontsize=legend_fontsize, ha="center")

    if len(names[1]) > 10:
        draw_text(fig, ax, 0.75, 0.7, names[1], colors[1], fontsize=legend_fontsize, ha="center", va="center")
    else:
        draw_text(fig, ax, 0.8, 0.7, names[1], colors[1], fontsize=legend_fontsize, ha="center", va="center")
...
def venn3(labels, names=['A', 'B', 'C'], **options):
    ...
    # legend
    if len(names[0]) > 10:
        draw_text(fig, ax, 0.25, 0.95, names[0], colors[0], fontsize=legend_fontsize, ha="center")
    else:
        draw_text(fig, ax, 0.2, 0.95, names[0], colors[0], fontsize=legend_fontsize, ha="center")

    if len(names[1]) > 10:
        draw_text(fig, ax, 0.75, 0.95, names[1], colors[1], fontsize=legend_fontsize, ha="center", va="center")
    else:
        draw_text(fig, ax, 0.8, 0.95, names[1], colors[1], fontsize=legend_fontsize, ha="center", va="center")

    if len(names[2]) > 10:
        draw_text(fig, ax, 0.25, 0.05, names[2], colors[2], fontsize=legend_fontsize, ha="center", va="center")
    else:
        draw_text(fig, ax, 0.2, 0.05, names[2], colors[2], fontsize=legend_fontsize, ha="center", va="center")
...
def venn4(labels, names=['A', 'B', 'C', 'D'], **options):
    ...
    # legend
    if len(names[0]) > 10:
        draw_text(fig, ax, 0.25, 0.05, names[0], colors[0], fontsize=legend_fontsize, ha="center")
    else:
        draw_text(fig, ax, 0.2, 0.05, names[0], colors[0], fontsize=legend_fontsize, ha="center")

    if len(names[1]) > 10:
        draw_text(fig, ax, 0.25, 0.95, names[1], colors[1], fontsize=legend_fontsize, ha="center", va="center")
    else:
        draw_text(fig, ax, 0.2, 0.95, names[1], colors[1], fontsize=legend_fontsize, ha="center", va="center")

    if len(names[2]) > 10:
        draw_text(fig, ax, 0.75, 0.95, names[2], colors[2], fontsize=legend_fontsize, ha="center", va="center")
    else:
        draw_text(fig, ax, 0.8, 0.95, names[2], colors[2], fontsize=legend_fontsize, ha="center", va="center")

    if len(names[3]) > 10:
        draw_text(fig, ax, 0.75, 0.05, names[3], colors[3], fontsize=legend_fontsize, ha="center", va="center")
    else:
        draw_text(fig, ax, 0.8, 0.05, names[3], colors[3], fontsize=legend_fontsize, ha="center", va="center")
    #leg = ax.legend(names, loc='center left', bbox_to_anchor=(1.0, 0.5), fancybox=True)
    #leg.get_frame().set_alpha(0.5)
